...
| D7 | L4 summary edges | **Own summary pass** (bottom-up over the SCC condensation, k-limited, monotone fixpoint) | Bottom-up composition with monotone fixpoint over SCC-condensation DAG, mirroring the reference analyzer's approach (which operates at statement granularity, not region decomposition); keystone-conformant. Heaviest L4 unit; lands last. |
